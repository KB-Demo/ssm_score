...
public class AdminExcelUtil {
    ...
    public void InfoExcelByPageBean(HttpServletResponse response, PageBean pb) throws IOException {
        ...
        if ("stuPerson".equalsIgnoreCase(pb.getExcel())) {
            ...
            titleRow.createCell(7).setCellValue("账号");
            titleRow.createCell(8).setCellValue("密码");
            for (Object o : list) {
                Stu stu = (Stu) o;
                HSSFRow dataRow = sheet.createRow(sheet.getLastRowNum() + 1);
                ...
                dataRow.createCell(7).setCellValue(stu.getS_userName());
                dataRow.createCell(8).setCellValue(stu.getS_psw());
            }
            response.setHeader("Content-Disposition", "attachment;filename="
        ...
        if ("teacherPerson".equalsIgnoreCase(pb.getExcel())) {
            ...
            for (Object o : list) {
                ...
                dataRow.createCell(7).setCellValue(teacher.getT_userName());
                dataRow.createCell(8).setCellValue(teacher.getT_psw());
            }
            response.setHeader("Content-Disposition", "attachment;filename="
